import Vue from 'vue';
import confirm from './ifConfirm.vue';
let confirmConstructor = Vue.extend(confirm);
let theConfirm = function (text) {
  return new Promise((resolve, reject) => { //promise封装，confirm执行resolve，cancel执行reject
    let confirmDom = new confirmConstructor({
      el: document.createElement('div')
    })
    document.body.appendChild(confirmDom.$el); //new一个对象，然后插入body里面
    confirmDom.text = Object.assign( confirmDom.text,text);  //为了使confirm的扩展性更强，这个采用对象的方式传入，所有的字段都可以根据需求自定义
    confirmDom.confirm  = function () { //如果调用了ifConfirm中的confirm方法,则调用该方法
      resolve()
      confirmDom.isShow = false
    }
    confirmDom.cancel = function () {
      reject()
      confirmDom.isShow = false
    }
  })
}
export default theConfirm;
//暴露出去，别忘记挂载到vue的原型上
//  => 在main.js里面先引入 import theConfirm from './components/confirm/confirm.js'
//  => 再挂载 Vue.prototype.$confirm = theConfirm;
//在需要的地方直接用以下方法调用即可：
//  this.$confirm({
//   type:'提示',
//   msg:'是否删除这条信息？',
//   btn:{
//     ok:'yes',
//     no:'no'
//   }
// }).then(() => {
//   console.log('ok')
// })
// .catch(() => {
//   console.log('no')
// })